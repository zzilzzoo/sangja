...
<jsp:forward page="/signin?return_path=ar-managment"></jsp:forward>
</c:if>
<!-- Basic -->
<meta charset="UTF-8">
						...
						<div class="card card-modern">
							<div class="card-body">
								<div class="datatables-header-footer-wrapper">
									<form name="search" action="ar-managment" method="post">
												...
												<div
													...
													<div class="input-daterange input-group"
														...
														<span
															...
															name="sale_ymd_e" value="<fmt:formatDate value="${dateValue_e}" pattern="MM/dd/yyyy"/>" autocomplete="off" >
													</div>
												</div>
											...
											<c:forEach items="${salelist}" var="salelist"
												...
												<tr>
													<fmt:parseDate var="sale_ymd" value="${salelist.sale_ymd}"
														pattern="yyyy-MM-dd" />
													<td><input type="hidden" id="sale_num"
														value="${salelist.sale_num}" />${status.count }</td>
													...
													<td><c:if test="${salelist.tot_pay_amt > 0 }">
															<a class="simple-ajax-popup"
																href="ajax-payment?sale_num=${salelist.sale_num}"><strong><fmt:formatNumber
																	value="${salelist.tot_pay_amt}" pattern="#,##0.00" /></strong></a>
														</c:if> <c:if test="${salelist.tot_pay_amt eq 0 }">
															<fmt:formatNumber value="${salelist.tot_pay_amt}"
																pattern="#,##0.00" />
															</strong>
														</c:if></td>
													...
													<!-- 연체일자 -->
													<fmt:parseDate
														value="${fn:substring(salelist.sale_ymd,0,10)}"
														var="strPlanDate" pattern="yyyy-MM-dd" />
													<fmt:parseNumber
													...
													<td class="text-center"><a
														class="image-popup-vertical-fit btn btn-dark "
														href="../resources/upimg/${sfvo.save_file_nm}"
														title="사진찍은 인보이스 이미지"> View </a></td>
													<td class="text-center"><button class="btn btn-dark"
															onclick="mkPrnFile('${salelist.sale_num}');return false">
															Reprint</button></td>
													<td>On-Site</td>
													<td class="text-center"><a href="#payoffForm"
														target="_blank" class="btn btn-success popup-with-form"
														onclick="setBalance(this);return false;"> Pay Off </a></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
									...
									<div id="payoffForm"
										class="modal-block modal-block-md mfp-hide">
										<section class="card">
											<header class="card-header">
											...
											<div class="card-body">
												<h3 class="text-center font-weight-bold text-dark">
													남은잔액 : <span class="text-color-danger" id="pay_amt">
													</span>
												</h3>
												<div class="row justify-content-center my-4">
													<div class="col-lg-3">
														<div class="radio-custom radio-primary">
															<input name="pay_type" type="radio" id="cash"
																checked="checked" value="cash" onchange="setDisplay()">
															<label for="cash" class="text-4">Cash</label>
														</div>
													</div>
													<div class="col-lg-4">
														<div class="radio-custom radio-primary">
															<input id="moneyorder" name="pay_type" type="radio"
																value="moneyorder" onchange="setDisplay()"> <label
																for="moneyorder" class="text-4">Money Order</label>
														</div>
													</div>
													<div class="col-lg-3">
														<div class="radio-custom radio-primary form-check-inline">
															<input id="check" name="pay_type" type="radio"
																value="check" onchange="setDisplay()"> <label
																for="check" class="text-4">Check</label>
														</div>
													</div>
												</div>
															...
															<div class="col-lg-6">
																<div class="input-group my-2">
																	<span class="input-group-text">$</span> <input
																		type="text" name="pay_amt1" id="pay_amt1"
																		class="form-control" value="18.50"> <input
																		type="hidden" name="pay_sale_num" id="pay_sale_num">
																</div>
															</div>
															<button class="col-lg-4 btn btn-success my-2"
																onclick="payPost();return false;">Pay</button>
														</div>
													</div>
												</div>

												<!-- check -->
												<div id="checkpayment" class="row justify-content-center"
													style="display: none">
													<div class="col-lg-10 alert alert-default">
														<h4 class="font-weight-bold text-dark">Payment Amount</h4>
														<div class="row justify-content-center">
															<div class="col-lg-auto">
																<div class="input-group my-2">
																	<span class="input-group-text text-2">Check #</span> <input
																		type="text" class="form-control" name="check_no"
																		id="check_no" maxlength="4" style="width: 80px">
																</div>
															</div>
															<div class="col-lg-4">
																<div class="input-group my-2">
																	<span class="input-group-text">$</span> <input
																		type="text" class="form-control" name="pay_amt2"
																		id="pay_amt2" value="18.50">
																</div>
															</div>
