...
<link rel="stylesheet"
	href="../resources/vendor/bootstrap-fileupload/bootstrap-fileupload.min.css" />
...
<body>
							...
							<div class="card card-modern">
								...
								<div class="card-body">
									<div class="row">
										<div class="col-lg-12">
											<ul
												class="g-info list list-unstyled list-item-bottom-space-0">
												<li><label>Order #</label> <span>${salevo.sale_num}</span>
												</li>
												<li><label>Sales Type</label> <span>${salevo.sale_type}</span>
													<span><input type="text"
														class="form-control form-control-sm"></span></li>
												<li><label>Sales Date</label> <c:set var="sale_ymd"
														value="${salevo.sale_ymd}" /> <fmt:parseDate
														var="sale_ymd" value="${sale_ymd}" pattern="yyyy-MM-dd" />
													<c:set var="sale_ymd">
														<fmt:formatDate value="${sale_ymd}" pattern="dd/MM/yyyy" />
													</c:set> <span>${sale_ymd} ${fn:substring(salevo.sale_time,0,5) } </span></li>
												<li><label>Sales Person</label> <span>${user_nm}</span>
												</li>
												<li><label>Sales Status</label> <span> <select
														class="form-control form-control-sm">
															<c:choose>
																...
																<c:when
																	test="${salevo.tot_ord_amt < salevo.tot_pay_amt &&  salevo.tot_pay_amt>0 }">
								...
								<div class="card-body">
									<div class="row">
										<div class="col-lg-12 pb-3">
											<ul
												class="c-info list list-unstyled list-item-bottom-space-0">
												...
												<table class="table table-responsive-md mb-0"
													...
													<tbody>
														...
														<c:forEach items="${spvoList}" var="spvoList"
															...
															<tr>
																...
																<td><fmt:formatNumber
																		value="${spvoList.unit_price}" pattern="#,##0.00" /></td>
																<td><fmt:formatNumber
																		value="${spvoList.sale_price}" pattern="#,##0.00" /></td>
																<td>${spvoList.sale_qty}</td>
																<td><fmt:formatNumber
																		value="${spvoList.tot_sale_price}" pattern="#,##0.00" /></td>
															</tr>
													...
													<tfoot>
														<tr>
															...
															<td colspan="2"><fmt:formatNumber value="${tot_amt}"
																	pattern="#,##0.00" /></td>
														</tr>
														<tr>
															<td colspan="5" class="border-0"></td>
															<td align="center" class="font-weight-bold">Sales
																Tax</td>
															<td align="center"></td>
															<td colspan="2"><fmt:formatNumber
																	value="${salevo.tot_tax_amt}" pattern="#,##0.00" /></td>
														</tr>
														<tr>
															<td colspan="5" class="border-0"></td>
															<td align="center" class="font-weight-bold">Shopping
																Cost</td>
															<td align="center"></td>
															<td colspan="2"><fmt:formatNumber
																	value="${salevo.shopping_cost}" pattern="#,##0.00" /></td>
														</tr>
														<tr>
															<td colspan="5" class="border-0"></td>
															<td align="center" class="font-weight-bold">Order
																Total</td>
															<td align="center" class="font-weight-bold">${tot_qty}</td>
															<td colspan="2" class="font-weight-bold"><fmt:formatNumber
																	value="${salevo.tot_ord_amt}" pattern="#,##0.00" /></td>
														</tr>
													</tfoot>
												</table>
											</div>
														...
														<tbody>
															...
															<c:forEach items="${payList}" var="payList">
																<tr>
																	...
																	<td>$ <fmt:formatNumber
																		value=" ${payList.pay_amt}" pattern="#,##0.00" /></td>
																	<td>${payList.pay_type}</td>
																</tr>
																<c:set var="tot_pay_amt"
																	value="${tot_pay_amt+payList.pay_amt }" />
															</c:forEach>
														</tbody>
													</table>
												</div>
											...
											<div class="row justify-content-end">
												<div class="col-auto">
													...
													<span class="d-flex align-items-center"> Total <i
														class="fas fa-chevron-right text-color-primary px-3"></i>
														<span
														class="d-flex align-items-center justify-content-lg-end">
															<strong class="text-color-danger text-6">$ <fmt:formatNumber
																	value="${salevo.tot_ord_amt - tot_pay_amt }" pattern="#,##0.00" /></strong>
													</span>
													</span>
												</div>
											</div>
	...
	<script
		src="../resources/vendor/bootstrap-fileupload/bootstrap-fileupload.min.js"></script>
