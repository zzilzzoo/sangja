					...
					<div class="row">
						<div class="col-lg-12">
							<ul class="g-info list list-unstyled list-item-bottom-space-0">
								...
								<li>
									<label>Order #</label>
									<span>${salevo.sale_num}</span>
								</li>
								<li>
									<label>Sales Type</label>
									<span>${salevo.sale_type}</span>
								</li>
								<li>
									<label>Sales Date</label>
									<c:set var="sale_ymd" value="${salevo.sale_ymd}" /> <fmt:parseDate
														var="sale_ymd" value="${sale_ymd}" pattern="yyyy-MM-dd" />
													<c:set var="sale_ymd">
														<fmt:formatDate value="${sale_ymd}" pattern="dd/MM/yyyy" />
													</c:set>
									<span>${sale_ymd} ${fn:substring(salevo.sale_time,0,5) }</span>
								</li>
								<li>
								...
								<li>
									...
									<c:if test="${salevo.tot_ord_amt >= salevo.tot_pay_amt }">
									<span>Completed</span>									
									</c:if>
									<c:if test="${salevo.tot_ord_amt < salevo.tot_pay_amt }">
					...
					<div class="row">
						<div class="col-lg-12">
							<ul class="c-info list list-unstyled list-item-bottom-space-0">
								<li>
									<label>Name</label>
									<span>${salevo.cust_nm}</span>
								</li>
								<li>
									<label>Address</label>
									<span>${salevo.addr}</span>
								</li>
								<li>
									<label>City</label>
									<span>${salevo.city}</span>
								</li>
								<li>
									<label>State</label>
									<span>${salevo.state}</span>
								</li>
								<li>
									<label>Zip Code</label>
									<span>${salevo.zip_code}</span>
								</li>
							</ul>
						</div>
					</div>
