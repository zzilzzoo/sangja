...
<c:if test="${sess_user==null }">
	<jsp:forward page="/signin?return_path=customer-list"></jsp:forward>
</c:if>
<!-- Basic -->
<meta charset="UTF-8">

<title>Customer List | SangJa Admin</title>
							...
							<div class="card-body">
								<div class="datatables-header-footer-wrapper">
									<div class="datatable-header">
										<form name="search" action="customer-list" method="post">
												...
												<div class="col-4 col-lg-auto ps-lg-1 mb-3 mb-lg-3">
													<div
														...
														<label class="ws-nowrap me-3 mb-0">Show:</label> <select
															class="form-control results-per-page" name="post_num">
												...
												<div class="col-6 col-lg-auto ps-lg-1 mb-3 mb-lg-3">
													<div
														...
														<label class="ws-nowrap me-3 mb-0">Filter By:</label> <select
															...
															<option value="cust_status">Status</option>
														</select>
													</div>
												</div>
										...
										<tbody>
											<c:forEach items="${list}" var="list" varStatus="status">
												<tr>
													...
													<td>${list.mng_user_nm}</td>
													<td><span class="ecommerce-status ${list.cust_status}">${list.cust_status}</span></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
									<div class="datatable-footer">
										...
										<!--<c:if test="${page.prev}">
											<span>[ <a
												href="customer-list?page_num=${page.startPageNum - 1}&search_user=${search_user}&filter_by=${filter_by}&search_term=${search_term}">이전</a>
												]
											</span>
										</c:if>

										<c:forEach begin="${page.startPageNum}"
											end="${page.endPageNum}" var="page_num">
											<span> <c:if test="${select != page_num}">
													<a href="customer-list?page_num=${page_num}&search_user=${search_user}&filter_by=${filter_by}&search_term=${search_term}">${page_num}</a>
												</c:if> <c:if test="${select == page_num}">
													<b>${page_num}</b>
												</c:if>
										...
										<c:if test="${page.next}">
											<span>[ <a
												href="customer-list?page_num=${page.endPageNum + 1}&search_user=${search_user}&filter_by=${filter_by}&search_term=${search_term}">다음</a>
												]
											</span>
										</c:if>-->
									</div>
