...
<form class="order-details action-buttons-fixed" method="post"
	id="unstoring" action="ajax-inventory-unstoring">
	<div class="row">
		<section class="card card-modern">
			<header class="card-header">
				<h2 class="card-title">Unstoring</h2>
			</header>
			<div class="card-body">
				<div class="row">
					...
					<div class="col-6 form-row">
						<label>옵션</label> <select class="form-control" id="select" name="event_opt"
							onchange="setDisplay();">
							<option>직원출고</option><!-- 회사재고(-),직원재고(+) -->
							<option>- 재고조정</option><!-- 회사재고(-) -->
							<option>Seller판매출고</option><!-- 회사재고(-),직원재고(+) -->
				...
				<div id="data1" class="row mb-3">
					<div class="col-6 form-row">
						<label>User</label> <select class="form-control" name="user_num1" id="user_num1">
					...
					<div class="col-6 form-row">
						<label>메모</label> <input type="text" class="form-control" name="event_note1">
					</div>
				</div>

				<div id="data2" class="row mb-3" style="display: none">
					<div class="col-6 form-row">
						<label>User</label> <input type="text" value="회사"
							class="form-control" readonly="readonly" name="user_num2" id="user_num2">
					</div>
					<div class="col-6 form-row">
						<label>메모</label> <input type="text" class="form-control" name="event_note2">
					</div>
				</div>

				<div id="data3" class="row mb-3" style="display: none">
					<div class="col-6 form-row">
						<label>User</label> <select class="form-control" name="user_num3" id="user_num3">
					...
					<div class="col-6 form-row">
						<label>메모</label> <input type="text" class="form-control" name="event_note3">
					</div>
				</div>

				<div id="data4" class="row mb-3" style="display: none">
					<div class="col-6 form-row">
						<label>User</label> <select class="form-control" name="user_num4" id="user_num4">
							<optgroup label="Individual Saller">
								<c:forEach items="${uListS}" var="uListS">
									<option value="${uListS.user_num}" selected>${uListS.user_nm_f}
										${uListS.user_nm_l}</option>
					...
					<div class="col-6 form-row">
						<label>메모</label> <input type="text" class="form-control" name="event_note4">
					</div>
				</div>
			...
			<footer class="card-footer">
				<div class="row">
					<div class="col-md-12 center">
						<button type="submit" form="unstoring" class="btn btn-primary">출고</button>
					</div>
				</div>
			</footer>
		</section>
...
<script>
								...
								.each(function(i) {
									...
									if (property == "직원출고") {
										trText = '<tr>' + '<td>'
												...
												+ '<td>'
												//cost_price
												+ '<input type="hidden" class="form-control" readonly="readonly"  name="InventoryInOutVOList['+trLen+'].cost_price" value="'+cost_price+'">'
												//unit_price
												+ '<input type="hidden" class="form-control" name="InventoryInOutVOList['+ trLen+ '].unit_price"  value="'+ unit_price+ '">'
												...
												+ '<td>'
												//총계
												+ '<input type="hidden" class="form-control"  name="InventoryInOutVOList['+trLen+'].event_amt" value="'+unit_price+'">'
	...
	function getVal(obj) {
		...
		var td = tr.children();
		var val1 = td.eq(6).children().children().eq(1).val();
		var val2 = td.eq(7).children().children().children().eq(1).val();
		//alert(val1);
		//alert(val2);

		var temp = uncomma(val1) * uncomma(val2)
		temp = commaStr(temp);
		td.eq(8).children().children().eq(1).val(temp);
